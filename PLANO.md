...
- [x] **Revisão de conformidade LGPD** (mapeamento documentado aqui, não uma auditoria externa formal):
  ...
  - **Achado que fica registrado, não corrigido:** `solicitacoes` e `usos` são legíveis por **qualquer conta `usuario`**, não só o dono do registro — ou seja, um condutor consegue ler `condutorNome`/`destino`/`motivo` de viagens de outros condutores. Isso é pré-existente (não introduzido nesta sessão) e existe porque `Solicitar.tsx` precisa checar conflito de horário num veículo, o que exige ler solicitações de outras pessoas. **Dá pra apertar** trocando essa checagem pra ler de `calendarioPublico` (que já tem `veiculoPlaca`/`dataSaida`/`dataRetorno`/`status` sem dado sensível) — mas isso muda comportamento: `calendarioPublico` só ganha um documento quando a solicitação é `aprovada` (não `pendente`), então duas pessoas pedindo o mesmo veículo no mesmo horário deixariam de ser bloqueadas até a primeira ser aprovada. Essa é uma troca de comportamento real, não um ajuste de regra — fica como decisão pendente do usuário, não uma coisa pra eu mudar sozinho.
